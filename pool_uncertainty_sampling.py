...
from modAL.uncertainty import uncertainty_sampling, classifier_uncertainty, classifier_margin, classifier_entropy, entropy_sampling, margin_sampling 
...
# Learning using Least Confident sampling strategy
al_LC = ActiveLearner(query_strategy= uncertainty_sampling, X_training=X_train, y_training=Y_train, 
                        estimator= knn )

# Learning using Entropy sampling strategy
al_H = ActiveLearner(query_strategy= entropy_sampling, X_training=X_train, y_training=Y_train, 
                      estimator= knn )

# Learning using Margin sampling strategy
al_M = ActiveLearner(query_strategy= margin_sampling, X_training=X_train, y_training=Y_train, 
                        estimator= knn )
...
for i in range(0,queries):
    query_idx, query_entry = al_model.query(X_unlab)
    
    X_new = X[unlabelled_idx[query_idx.item()]].reshape(1,-1)
    Y_new = Y[unlabelled_idx[query_idx.item()]].reshape(1,)
